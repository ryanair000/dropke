...
import { initializePaystack, paystackMode } from '@/lib/paystack';
...
export async function POST(request: Request) {
  try {
    ...
    if (!['test', 'live'].includes(paystackMode())) return Response.json({ error: 'Paystack is not configured' }, { status: 503 });
