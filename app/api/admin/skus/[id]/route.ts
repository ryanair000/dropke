...
export async function PUT(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const { data, error } = await supabase.from('gift_card_skus').update({ sell_price_kes: Math.round(sellPriceKes), low_stock_threshold: lowStockThreshold, updated_at: new Date().toISOString() }).eq('id', params.id).select('id,sku,sell_price_kes,low_stock_threshold').single();
